...
export class ConfirmTransactionController {
  async handle(req: Request, res: Response) {
    ...
    try {
      const confirmTransactionService = new ConfirmTransactionService()
      const transaction = await confirmTransactionService.execute({ transaction_id, source: "manual", performed_by: id, reason })

      return res.status(200).json({ message: "Transaction Is Now Confirmed", transaction })
    } catch (error: unknown) {
