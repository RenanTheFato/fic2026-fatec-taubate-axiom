...
interface RefuseTransactionServiceProps {
...
export class RefuseTransactionService {
  async execute({ transaction_id, source, performed_by, reason }: RefuseTransactionServiceProps) {

    // Pagamento recusado, pelo gateway ou pela própria equipe. Nenhum efeito financeiro foi
    // aplicado ainda, então não há arrecadação nem vaga para reverter.
    return await sequelize.transaction(async (t) => {
      ...
      if (transaction.status !== "pending" && transaction.status !== "awaiting_confirmation") {
        throw new BadRequestError("Only a pending or awaiting confirmation transaction can be refused")
      }

      const previousStatus = transaction.status

      await transaction.update({ status: "refused" }, { transaction: t })
